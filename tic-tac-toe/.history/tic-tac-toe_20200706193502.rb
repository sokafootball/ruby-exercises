...
class Game
  ...
  private
  @@players = [Constants::HUMAN, Constants::COMPUTER]
  @@moving_player = nil
  @@winning_player = nil
  PLAYER_SYMBOL = "O"
  COMPUTER_SYMBOL = "X"
  ...
  def Game.end_game
    if (@@winning_player == Constants::HUMAN)
      puts "You won!"
  ...
  def Game.run_turn
    if @@moving_player == Constants::HUMAN
      run_player_turn
    else
      run_computer_turn
    end
    @@moving_player = @@moving_player == Constants::HUMAN ? Constants::COMPUTER : Constants::HUMAN
  end
  ...
  def Game.set_winner(token)
    if token == PLAYER_SYMBOL
      @@winning_player = Constants::HUMAN
    else
