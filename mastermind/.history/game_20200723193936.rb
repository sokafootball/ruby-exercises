...
class Game
  ...
  SIGNS = []
  (1..Options::NUMBER_OF_SIGNS).each {|num| SIGNS << num}
  ...
  def self.generate_solution
    @@solution = SIGNS.shuffle.first(Options::COMBINATION_LENGTH)
  end
  ...
  def self.get_player_guess
    @@player_guess = []
    @@available_SIGNS = SIGNS.values_at(0...Options::NUMBER_OF_SIGNS)
